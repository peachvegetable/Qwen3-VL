#!/usr/bin/env python3
"""
Qwen3-VL Attention Extraction

Extract and visualize first-layer prefill attentions directly from native Qwen3-VL
checkpoints (no StreamingVLM dependency). The script mirrors the original
`extract_attention.py` features:

- Text→Vision attention (averaged across heads)
- Vision→Text attention (averaged across heads) + optional per-head dumps (no averaging)
...
Quickstart:
    python extract_attention_qwen3vl.py \\
        --model_path Qwen/Qwen3-VL-4B-Instruct \\
        --video_path demo.mp4 \\
        --question "What is happening?" \\
        --output_dir outputs/qwen3vl_demo

Output Structure (identical to StreamingVLM script):
    outputs/qwen3vl_demo/
    ├── prefill_attention.pt
    ├── frame_000_text_to_vision.npy
    ├── frame_000_vision_to_text.npy
    ├── frame_000_vision_to_text_head00.npy        # if --vision_to_text
    ├── frame_000_head00_vision_to_text.png        # if --visualize_per_head
    ├── per_head_tv/..., per_head_tv_heatmaps/...  # per-head TV dumps
    ├── sampled_frames/*.jpg                       # frames fed to the model
    └── metadata.json
"""
...
def load_model_and_processor(
    ...
    """Load native Qwen3-VL model + processor."""
    if not HAS_QWEN_VL_UTILS:
        raise ImportError("qwen_vl_utils>=0.0.14 is required for video preprocessing")
...
def _resolve_layers_container(model) -> Tuple[torch.nn.Module, List[torch.nn.Module]]:
    """Locate the decoder layer list regardless of HF wrapper naming."""
    candidate_paths = [
    ...
    for path in candidate_paths:
        ...
        # Expect a list/ModuleList of decoder blocks
        layers = node
        try:
            length = len(layers)
        except TypeError:
        ...
        if hasattr(first, "self_attn"):
            return layers
    raise RuntimeError("Unable to locate decoder layers with a self_attn module on the loaded model.")
...
def extract_attention_qwen3vl(
    ...
    print("=" * 80)
    print("QWEN3-VL ATTENTION EXTRACTION")
    print("=" * 80)
    ...
    print(f"Device: {target_device} | dtype: {dtype}")

    # Register attention hook
    print(f"[2/5] Registering attention hook (capture_decode={capture_decode})")
    capture = AttentionCapture(capture_decode, max_decode_steps)
    ...
    # Generate
    print(f"[4/5] Generating with Qwen3-VL")
    ...
    # Extract attention
    print(f"[5/5] Extracting and visualizing attention\n")
    ...
    try:
        ...
        # Also print text-only row mapping and save CSV for easy lookup in plots
        print("\nText-only index mapping (row -> abs_index, piece):")
        mapping_rows = []
        for r, abs_idx in enumerate(text_idx.tolist()):
            tok_id = int(ids_row[abs_idx])
            piece = processor.tokenizer.decode([tok_id])
            print(f"{r} -> {abs_idx}, {piece!r}")
            mapping_rows.append({
                "row": r,
                "abs_index": abs_idx,
                "token_id": tok_id,
        ...
        try:
            import csv
            with open(output_dir / "text_token_map.csv", "w", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=["row", "abs_index", "token_id", "piece"])
                writer.writeheader()
                for row in mapping_rows:
                    writer.writerow(row)
            print(f"Saved text token map to {output_dir / 'text_token_map.csv'}")
        except Exception as e:
            print(f"Warning: failed to save text_token_map.csv: {type(e).__name__}: {e}")
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Extract first-layer attention maps from Qwen3-VL")
    ...
    parser.add_argument(
        ...
        help="Attention implementation to request when loading the model (set '' to disable)",
    )
